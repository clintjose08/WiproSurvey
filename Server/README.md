...
Open [http://10.201.174.210:3000](http://10.201.174.210:3000) to view it in the browser.
...
```js
  "proxy": "http://10.201.174.210:4000",
```

This way, when you `fetch('/api/todos')` in development, the development server will recognize that it’s not a static asset, and will proxy your request to `http://10.201.174.210:4000/api/todos` as a fallback. The development server will only attempt to send requests without a `text/html` accept header to the proxy.

Conveniently, this avoids [CORS issues](http://stackoverflow.com/questions/21854516/understanding-ajax-cors-and-security-considerations) and error messages like this in development:

```
Fetch API cannot load http://10.201.174.210:4000/api/todos. No 'Access-Control-Allow-Origin' header is present on the requested resource. Origin 'http://10.201.174.210:3000' is therefore not allowed access. If an opaque response serves your needs, set the request's mode to 'no-cors' to fetch the resource with CORS disabled.
```
...
HOST | :white_check_mark: | :x: | By default, the development web server binds to `10.201.174.210`. You may use this variable to specify a different host.
